...
class Image:
    name: str  # 画像の名前
    url: list  # 画像のURL (str型)
    cv: list  # cv2で読み込んだ画像 (str型)
...
# 時間割の画像のみ抽出
now = Image(name="now", url=[], cv=[])
latest = Image(name="latest", url=[], cv=[])
for index, e in enumerate(imgs_tag, 1):
    logger.info(f"{index}枚目: {e}")
    if str(cv2u.urlread(e)) not in now.cv:
        logger.info(" → append")
        now.cv.append(str(cv2u.urlread(e)))
        now.url.append(e)
logger.info(f"現在の画像:{now.url}")

# $GITHUB_OUTPUTに追加
subprocess.run([f"echo NOW={",".join(now.url)} >> $GITHUB_OUTPUT"], shell=True)
...
time.sleep(2)
try:
    latest.url = ws.acell("C6").value.split()    # URLリスト(過去)
except:
    logger.warning("Googleスプレッドシートへのアクセス失敗\n")
    subprocess.run(["echo STATUS=Googleスプレッドシートへのアクセス失敗 >> $GITHUB_OUTPUT"], shell=True)
    exit()
logger.info(f"過去の画像:{latest.url}\n")
for e in latest.url:
    latest.cv.append(str(cv2u.urlread(e)))

# $GITHUB_OUTPUTに追加
before = ",".join(latest.url)
subprocess.run([f"echo BEFORE={before} >> $GITHUB_OUTPUT"], shell=True)

# 更新通知のチェック
if ws.acell("D3").value == "NoUpdate":
    # 比較
    if len(now.url) == len(latest.url):
        if set(now.cv) == set(latest.cv):
            finish("画像が一致した為、終了")
        else:
            logger.info("画像が一致しないので続行")
    else:
        if len(now.url) < len(latest.url) and set(now.cv).issubset(latest.cv):
...
for i in now.url:
    ...
    img = str(now.url.index(i)) + ".png"    # 画像の名前を0.png,1.png,...とする
